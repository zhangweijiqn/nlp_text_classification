...
# input: 字符/词 index，卷积核相当于是n-gram，embedding layer可以使用word2vec预训练，也可以是单独一个网络
# print("started...")
    ...
    def __init__(self, filter_sizes, num_filters, num_classes, learning_rate, batch_size, decay_steps, decay_rate,
        ...
        if not self.multi_label_flag:
            ...
            correct_prediction = tf.equal(tf.cast(self.predictions, tf.int32),
                                          tf.cast(self.input_y, tf.int32))  # tf.argmax(self.logits, 1)-->[batch_size]
        ...
        with tf.name_scope("embedding"):  # embedding matrix
            self.Embedding = tf.get_variable("Embedding", shape=[self.vocab_size, self.embed_size],
                                             initializer=tf.random_normal_initializer(
                                                 stddev=0.1))  # [vocab_size,embed_size] tf.random_uniform([self.vocab_size, self.embed_size],-1.0,1.0)
            self.W_projection = tf.get_variable("W_projection", shape=[self.num_filters_total, self.num_classes],
                                                initializer=tf.random_normal_initializer(
                                                    stddev=0.1))  # [embed_size,label_size]
    ...
    def cnn_multiple_layers(self):
        ...
        for i, filter_size in enumerate(self.filter_sizes):
            with tf.variable_scope('cnn_multiple_layers' + "convolution-pooling-%s" % filter_size):
                ...
                # 2) CNN->BN->relu
                h = tf.reshape(h, [-1, self.sequence_length, self.num_filters,
                                   1])  # shape:[batch_size,sequence_length,num_filters,1]
                # Layer2:CONV-RELU
                filter2 = tf.get_variable("filter2-%s" % filter_size,
                                          [filter_size, self.num_filters, 1, self.num_filters],
                                          initializer=self.initializer)
                conv2 = tf.nn.conv2d(h, filter2, strides=[1, 1, 1, 1], padding="SAME",
                ...
                pooling_max = tf.squeeze(
                    tf.nn.max_pool(h, ksize=[1, self.sequence_length, 1, 1], strides=[1, 1, 1, 1], padding='VALID',
                                   name="pool"))
...
def test():
    ...
    tc = TextCNN(filter_sizes, num_filters, num_classes, learning_rate, batch_size, decay_steps, decay_rate,
    ...
    with tf.Session() as sess:
        ...
        for i in range(500):
            ...
            print(input_x)  # feature 是 word/char index
            input_x[input_x >= 0] = 1
            input_x[input_x < 0] = 0
            input_y_multilabel = get_label_y(input_x)
            print(input_y_multilabel)  # label 是 multi-hot
            loss, possibility, W_projection_value, _ = sess.run(
                [tc.loss_val, tc.possibility, tc.W_projection, tc.train_op],
                feed_dict={tc.input_x: input_x, tc.input_y_multilabel: input_y_multilabel,
                           tc.dropout_keep_prob: dropout_keep_prob, tc.tst: False, tc.is_training_flag: True})
            print(i, "loss:", loss, "-------------------------------------------------------")
